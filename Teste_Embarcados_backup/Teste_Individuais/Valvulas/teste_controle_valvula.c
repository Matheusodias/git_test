...
void set_pressure(int signo){
	...
	if(pressure > 0){
		...
		//printf("Controle de alta pressão acionado!");
		//printf("Pressão setada: %f", pressure);
		//printf("Pressão medida: %f", normal_load);
		
		if(normal_load < (pressure - (hist/2))){
			system("echo 1 > /sys/class/gpio/gpio16/value");
			system("echo 0 > /sys/class/gpio/gpio20/value");
		}
		else if(normal_load > (pressure + (hist/2))){
			system("echo 0 > /sys/class/gpio/gpio16/value");
			system("echo 1 > /sys/class/gpio/gpio20/value");
		}
		else if(normal_load > (pressure - (hist/2)) && normal_load < (pressure + (hist/2))){
			system("echo 0 > /sys/class/gpio/gpio16/value");
			system("echo 0 > /sys/class/gpio/gpio20/value");
		}
	}
